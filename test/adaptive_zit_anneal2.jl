# Adaptive ZIT — Signal-Gated Annealing, asymmetric rates.
#
# Same design as adaptive_zit_anneal.jl but with COOL_RATE < HEAT_RATE.
# This gives heating a structural advantage so thin markets (frequent negative
# marginal surplus under standard ZIT) converge to higher λ_eq.
#
# Equilibrium condition (per agent):
#   λ_eq × P(sig>0) × α  =  (1-λ_eq)² × P(sig<0|std) × β
#
# With α=0.01, β=0.05 and equal P(pos)≈P(neg) (n_active/n_total ≈ 0.5):
#   λ_eq ≈ 5×(1-λ_eq)² / [λ_eq×0.37 + (1-λ_eq)×0.17]  →  λ_eq ≈ 0.74
#
# With very thin markets (n_active/n_total → 0): P(neg|std) → 1 → λ_eq → 1
# With very thick markets (n_active/n_total → 1): P(neg|std) → 0 → λ_eq → 0
#
# Extended run (T=600) to confirm convergence of small markets.
#
# Run with:
#   julia --project=. test/adaptive_zit_anneal2.jl
...
const T_TURNS   = 600
const MAX_UNITS = 5
const COOL_RATE = 0.01   # slow cooldown — heating can overcome in thin markets
const HEAT_RATE = 0.05   # fast heatup  — quickly responds to overtrading
...
function simulate_market(consumers::Vector{AdaptConsumer},
    ...
    for t in 1:T_TURNS
        ...
        for i in 1:nc
            ...
            λ_c[i] = update_λ(λ_c[i], marginal_surplus_c(consumers[i], r))
        end
        for j in 1:nf
            r = realized_f(draws_f[j], D, S)
            λ_f[j] = update_λ(λ_f[j], marginal_surplus_f(firms[j], r))
        end
    end
...
function run_adaptive(; nc, nf, n_markets, seed_base, label, max_units=MAX_UNITS,
                       report_turns=[1,2,5,10,25,50,100,200,300,400,500,600])
    ...
    println("=" ^ 72)
    println(label)
    @printf("  n=%d  T=%d  cool=%.3f  heat=%.3f\n\n",
...
println("ADAPTIVE ZIT — Signal-Gated Annealing v2  (asymmetric rates)")
println("  cool=$(COOL_RATE)  heat=$(HEAT_RATE)  →  heating has $(Int(HEAT_RATE/COOL_RATE))× structural advantage")
println("  Thin markets (P(neg|std) high): heating wins  →  λ_eq → high")
println("  Thick markets (P(neg|std) ≈ 0): cooling wins  →  λ_eq → 0")
println("  T=$(T_TURNS) turns (extended from 300 to confirm thin-market convergence)")
println()

RT = [1, 2, 5, 10, 25, 50, 100, 200, 300, 400, 500, 600]

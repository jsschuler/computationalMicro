# Adaptive ZIT with MARGINAL-UNIT SURPLUS as the performance signal.
#
# Performance signal = surplus of the last (marginal) unit traded:
#   Consumer assigned k units: signal = full_wtp[k] - p*
#   Firm     assigned k units: signal = p* - full_wtac[k]
#   k = 0: signal = 0.0  (no trade)
#
# full_wtp is sorted descending (all units, including wtp < p*).
# full_wtac is sorted ascending (all units, including wtac > p*).
#
# Key property:
#   Standard ZIT consumer can draw k > n_active → full_wtp[k] < p* → signal < 0.
#   BiGeo consumer draws from {0,...,n_active} → full_wtp[k] ≥ p* → signal ≥ 0.
#   This gives consumers a proper informational signal about unit quality.
#
# Prediction: consumers with large (n_total - n_active) strongly prefer BiGeo
# because standard frequently returns negative marginal surplus. Thin markets
# (high p*, few active units) should see high BiGeo adoption.
#
# Run with:
#   julia --project=. test/adaptive_zit_marginal.jl
...
struct AdaptConsumer
    ...
    full_wtp :: Vector{Float64}   # all wtp, sorted descending (length = n_total)
    p_star_f :: Float64
end
...
struct AdaptFirm
    ...
    full_wtac :: Vector{Float64}   # all wtac, sorted ascending (length = n_total)
    p_star_f  :: Float64
end
...
function simulate_market(consumers::Vector{AdaptConsumer},
    ...
    for t in 1:T_TURNS
        ...
        # Performance update: marginal-unit surplus
        for i in 1:nc
            r   = realized_c(draws_c[i], D, S)
            sig = marginal_surplus_c(consumers[i], r)
            if use_bgip_c[i]; perf_bgip_c[i] = sig
            else;              perf_std_c[i]  = sig; end
        end
        for j in 1:nf
            r   = realized_f(draws_f[j], D, S)
            sig = marginal_surplus_f(firms[j], r)
            if use_bgip_f[j]; perf_bgip_f[j] = sig
            else;              perf_std_f[j]  = sig; end
        end
...
println("  Consumer: full_wtp[k] - p*  (negative when k > n_active: overtrading)")
println("  Firm:     p* - full_wtac[k] (negative when k > n_active: underselling)")
println()
